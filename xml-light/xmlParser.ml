...
let pop s =
  try Stack.pop s.stack with
  | Stack.Empty -> Xml_lexer.token s.source
;;

let push t s = Stack.push t s.stack
...
and read_elems ?tag s =
  ...
  (try
     while true do
       ...
       | true, PCData c, PCData c2 :: q -> elems := PCData (sprintf "%s\n%s" c2 c) :: q
       | _, x, l -> elems := x :: l
     done
   with
   | NoMoreData -> ());
...
let read_xml s =
  ...
  | true, Xml_lexer.DocType (root, Xml_lexer.DTDData dtd) ->
    ...
    let x = read_node s in
    Dtd.prove dtd root x
  | false, Xml_lexer.DocType _ -> read_node s
  | _, t ->
    push t s;
    read_node s
;;
...
let do_parse xparser source =
  try
    ...
    let s = { source; xparser; stack = Stack.create () } in
    let tk = pop s in
    (* skip UTF8 BOM *)
