...
let load_rows_inchar ?(separator = ',') f inchar =
  ...
  let rec loop () =
    ...
    if c <> '\r'
    then (
      (* Always ignore \r characters. *)
      match !state with
      | StartField ->
        (* Expecting quote or other char. *)
        if c = '\"'
        then (
          state := InQuotedField;
          field := [])
      ...
      | InQuotedField ->
        (* Reading chars to end of field. *)
        if c = '\"' then state := InQuotedFieldAfterQuote else field := c :: !field
...
let trim ?(top = true) ?(left = true) ?(right = true) ?(bottom = true) csv =
  ...
  in
  let empty_left_cell = function
    | [] -> true
    | x :: _ when x = "" -> true
    | _ -> false
  in
  let empty_left_col = List.fold_left (fun a row -> a && empty_left_cell row) true in
  let remove_left_col =
    List.map (function
      | [] -> []
      | _ :: xs -> xs)
...
let save_out ?separator chan csv = save_fn ?separator (Out_channel.output_string chan) csv

let print ?separator csv =
  save_out ?separator Out_channel.stdout csv;
  ...
  let widths =
    ...
    | r :: _ ->
      ...
      let rec repeat x = function
        ...
        | i -> x :: repeat x (i - 1)
      in
      List.fold_left max2rows (repeat 0 n) lengths
  in
